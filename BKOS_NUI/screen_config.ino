...
// Alle snelle naam chips in twee rijen
...
static void helderheid_balk_teken() {
    ...
    tft.setTextSize(1); tft.setTextColor(C_TEXT_DIM);
    tft.setCursor(bx + 4, HLD_Y + 4); tft.print("HELDER");
}
...
struct IoPreset {
    ...
    const char* kanalen[32];  // nullptr = einde
};
...
static void preset_toepassen(int idx) {
    ...
    if (idx == N_PRESETS - 1) {
        // Wissen: zet alles terug naar "IO X"
...
void screen_config_run(int x, int y, bool aanraking) {
    ...
    if (y >= CFG_IO_Y + rijen_n * CFG_RIJ_H && y < (int)(TFT_H - NAV_H)) {
        ...
        else cfg_scroll = min(max(0, io_kanalen_cnt - rijen_n), cfg_scroll + rijen_n);
        screen_config_rijen_teken();
    }
}
